...
namespace GameServer.Api.Core;

/// <summary>
/// 统一业务调用入口。
/// Godot客户端所有操作必须通过此接口调用。
/// 调用流程：Godot C#脚本 → IGameServerApi（异步方法） → 服务端内部（MediatR → Orleans Grain） → 返回DTO/事件。
/// </summary>
public interface IGameServerApi
{
    ...
    Task<IReadOnlyList<MarketOrderDto>> GetMarketOrdersAsync(string stationId, string? itemFilter, CancellationToken ct = default);
    ...
    Task<CraftingJobDto> StartCraftingAsync(Guid playerId, string blueprintId, int quantity, CancellationToken ct = default);
    ...
    Task<CreateGuildResultDto> CreateGuildAsync(CreateGuildCommandDto cmd, CancellationToken ct = default);
    ...
    Task<AcceptQuestResultDto> AcceptQuestAsync(Guid playerId, string questId, bool trackProgress, CancellationToken ct = default);
    ...
    // ── Legacy scripting (kept for compatibility) ─────────────────────
    ...
    Task<ScriptResultDto> ChooseDialogueOptionAsync(Guid playerId, string scriptId, int optionIndex, CancellationToken ct = default);
